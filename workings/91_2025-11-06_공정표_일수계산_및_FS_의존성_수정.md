...
## 최종 수정 (2차 수정 - 2025-11-06)

### `addWorkingDays` 함수 로직 재수정

첫 번째 수정에서 근본적인 오류가 발견되어 다시 수정했습니다.

#### 발견된 문제
첫 번째 수정 후에도 여전히 날짜가 잘못 계산되었습니다:
- cw-002: 11월 6일 시작, 14일 소요
- 로그: 11월 22일 종료 (13일차)
- 예상: 11월 25일 종료 (14일차)

**원인**: `addWorkingDays` 함수가 시작일을 카운트에서 **제외**하고 있었습니다.
- 기존 로직: 시작일 다음 날부터 N일 카운트
- 올바른 로직: 시작일을 1일차로 포함하여 N일차 계산
...
function addWorkingDays(startDate, workingDays, calendar = null) {
    ...
    if (workingDays === 0) {
        ...
        console.log(`[DEBUG][addWorkingDays] Lag=0: ${startDate} → ${current}`);
        return current;
    }

    // ★ 핵심: startDate를 1일차로 포함하여 N일차 계산
    let current = new Date(startDate);
    let daysAdded = 0;
    ...
    if (isWorkingDay(current, calendar)) {
        daysAdded = 1;
        console.log(`[DEBUG][addWorkingDays] Day ${daysAdded}: ${current}`);
    }
    ...
    console.log(`[DEBUG][addWorkingDays] RESULT: ${startDate} 시작, ${workingDays}일 소요 = ${current} 종료`);
    return current;
}
```
...
#### 검증 로그
```
[DEBUG][addWorkingDays] Day 1: 2025-11-06
[DEBUG][addWorkingDays] Day 2: 2025-11-07
[DEBUG][addWorkingDays] Day 3: 2025-11-10
[DEBUG][addWorkingDays] Day 4: 2025-11-11
[DEBUG][addWorkingDays] Day 5: 2025-11-12
[DEBUG][addWorkingDays] Day 6: 2025-11-13
[DEBUG][addWorkingDays] Day 7: 2025-11-14
[DEBUG][addWorkingDays] Day 8: 2025-11-17
[DEBUG][addWorkingDays] Day 9: 2025-11-18
[DEBUG][addWorkingDays] Day 10: 2025-11-19
[DEBUG][addWorkingDays] Day 11: 2025-11-20
[DEBUG][addWorkingDays] Day 12: 2025-11-21
[DEBUG][addWorkingDays] Day 13: 2025-11-24
[DEBUG][addWorkingDays] Day 14: 2025-11-25
[DEBUG][addWorkingDays] RESULT: 2025-11-06 시작, 14일 소요 = 2025-11-25 종료
```
...
[DEBUG][addWorkingDays] Lag=0: 2025-11-25 → 2025-11-26
...
1. ✅ **공정계획/시뮬레이션 탭 일수 불일치**: Duration 집계 방식 수정으로 해결
2. ✅ **Lag 일수 올림 미적용**: 양수 ceil, 음수 floor 처리 완료
3. ✅ **FS + Lag=0 날짜 계산 오류**: addWorkingDays 로직 수정으로 완전 해결
4. ✅ **Duration 계산 오류**: 시작일을 1일차로 포함하도록 수정
...
1. 디버깅 로그 정리 (프로덕션 배포 시)
2. 다양한 시나리오 테스트 (SS, SF, FF 의존성)
3. 성능 최적화 검토
