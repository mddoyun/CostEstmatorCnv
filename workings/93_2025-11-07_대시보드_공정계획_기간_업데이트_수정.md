...
## 작업 개요
홈 탭 대시보드의 "공정계획" 카드에서 총 공사기간이 제대로 표시되지 않는 문제를 해결했습니다.
- 초기 문제: "데이터 없음" 표시
- 추가 문제: 공정계획 탭을 거치지 않고 홈 탭에서 바로 프로젝트를 선택하면 잘못된 기간 계산 (22일 vs 실제 53일)
...
## 추가 수정 3: 프로젝트 캘린더 로드 누락 문제

### 추가 문제 상황
프로젝트 시작일 설정 후에도 여전히 대시보드에서 잘못된 기간이 표시되는 문제가 발생했습니다.

**증상**:
- 홈 탭에서 바로 프로젝트 선택 시: "2025. 11. 7. ~ 2025. 11. 29. (22일)" - 잘못된 계산
- 공정계획 탭을 방문한 후: "2025. 11. 7. ~ 2025. 12. 30. (53일)" - 올바른 계산

**차이 분석**:
- 22일 = 단순 duration 합산 (선행관계, lag, offset 미반영)
- 53일 = 정확한 간트차트 계산 (선행관계, lag, offset 모두 반영)

### 원인 분석
`calculateTaskDates()` 함수는 프로젝트 캘린더 데이터를 필요로 합니다. 이 함수는 `isWorkingDay()` 함수와 `mainCalendar` 데이터를 사용하여 작업일을 기준으로 날짜를 계산합니다.

간트차트 탭에서는 `loadAndRenderGanttChart()` 함수가 처음에 `loadProjectCalendars()`를 호출하여 캘린더를 로드하지만 (line 161), 대시보드에서는 이 단계가 누락되어 있었습니다.

**캘린더가 없을 때의 영향**:
- `calculateTaskDates()`가 선행관계를 제대로 계산하지 못함
- Lag와 offset이 적용되지 않음
- 결과적으로 단순 duration 합산과 동일한 결과 산출

### 해결 방법

#### 1. loadProjectCalendars 함수 전역 노출
**파일**: `connections/static/connections/gantt_chart_handlers.js`
**위치**: Line 1786
...
window.generateGanttData = generateGanttData;
window.calculateTaskDates = calculateTaskDates;
window.loadProjectCalendars = loadProjectCalendars;  // 추가
```

**효과**: 대시보드에서 프로젝트 캘린더를 로드할 수 있게 됨
...
**효과**:
- `calculateTaskDates()`가 작업일 기준으로 정확한 날짜 계산 가능
- 선행관계, lag, offset이 올바르게 적용됨
...
calculateScheduleDatesFromGantt()
  ├─ window.ganttMinDate 확인 → 없음
  ├─ ✓ 1. loadProjectCalendars() 호출 (mainCalendar 로드)
  ├─ ✓ 2. localStorage에서 프로젝트 시작일 읽기
  ├─ ✓ 3. window.projectStartDate 설정
  ├─ 4. CostItem 그룹핑
  ├─ 5. Activity/Dependency 추출
  ├─ 6. ActivityMap 생성
  └─ 7. generateGanttData() 호출
      ├─ projectStartDate 사용 ✓
      ├─ 간트 태스크 생성
      └─ 8. calculateTaskDates() 호출
          ├─ mainCalendar 사용 ✓ (작업일 체크)
          ├─ activityMap 사용 ✓
          ├─ 선행관계 계산 ✓
          ├─ offset/lag 적용 ✓
          └─ 정확한 날짜 계산 완료
...
**수정 전** (캘린더 미로드):
```
[Dashboard] Calculated dates using gantt logic: Fri Nov 07 2025 ~ Sat Nov 29 2025 - 22
→ 공정계획 카드: "2025. 11. 7. ~ 2025. 11. 29. (22일)" ❌
```

**수정 후** (캘린더 로드):
```
[Dashboard] Loading project calendars before gantt calculation
[Dashboard] Using project start date: 2025-11-06
[Dashboard] Calculated dates using gantt logic: Fri Nov 07 2025 ~ Tue Dec 30 2025 - 53
...
**간트차트 탭 초기화 순서** (비교용):
```javascript
// gantt_chart_handlers.js - loadAndRenderGanttChart()
async function loadAndRenderGanttChart() {
    // 1. 캘린더 로드
    await loadProjectCalendars(currentProjectId);  // line 161

    // 2. 프로젝트 시작일 설정
    projectStartDate = new Date(savedStartDate);   // line 168
...
**대시보드 초기화 순서** (수정 후):
```javascript
// home_tab_handlers.js - calculateScheduleDatesFromGantt()
async function calculateScheduleDatesFromGantt() {
    // 1. 캘린더 로드 (간트차트와 동일)
    await window.loadProjectCalendars(window.currentProjectId);  // line 713

    // 2. 프로젝트 시작일 설정 (간트차트와 동일)
    window.projectStartDate = projectStartDate;  // line 734

    // 3. 데이터 계산 (간트차트와 동일)
    const ganttTasks = window.generateGanttData(...);  // line 737
    window.calculateTaskDates(...);  // line 741
}
```

**일관성 확보**: 대시보드와 간트차트가 동일한 초기화 순서를 따르도록 수정하여 계산 결과의 일관성을 보장합니다.
